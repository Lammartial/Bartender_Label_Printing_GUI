...
if tool_col is None:
    raise ValueError(
        "Could not find a column containing 'Tool No.'.\n\n"
        f"Detected columns:\n{list(df.columns)}"
    )
...
print(f"Detected Tool No column: {tool_col}")
...
# Keep every tool, even if Inventory Number is blank.
# Only remove rows where the Q-ID itself is missing.
df = df[
    df["TOOL_NO"] != ""
]

# Remove duplicate tool/inventory combinations
df = df.drop_duplicates()

# Sort by tool number (optional)
df = df.sort_values(
    by="TOOL_NO"
...
print()
print("========================================")
print("Tool database created successfully.")
...
DATABASE_FILE = os.path.join(PROJECT_ROOT, "databases", "Fixture-Jig_DB.csv")
DB_DELIMITER = "\t"  
...
# GUI APP
# =========================
class Q_Tool_Menu(tk.Tk):
    def __init__(self):
        super().__init__()
        self.title("Fixture / Jig - Label Print System")
        ...
        self.fixture_jig_db = self.load_database()
        self.build_ui()

    def load_database(self):
        """Reads the tool database CSV and returns a dictionary mapped by TOOL_NO."""
        db = {}
    ...
    def build_ui(self):
        ...
        tk.Button(
            form_frame,
            text="Option 1: Print All Fixture/Jig Labels",
    ...
    def execute_batch_print(self):
        """
        Batch print every fixture/jig contained in Fixture-Jig_DB.csv.
        """

        jobs_list = []
        for tool_no, data in self.fixture_jig_db.items():
        ...
        if not jobs_list:
            ...
            messagebox.showwarning(
                "Database Empty",
                "Fixture-Jig_db.csv does not contain any tool."
            )
            return
        ...
        answer = messagebox.askyesno(
            ...
            "You are about to print ALL tool labels.\n\n"
        ...
        if success:
            messagebox.showinfo(
                ...
                f"Successfully queued {len(jobs_list)} tool labels for printing."
            )
...
class ManualPrintWindow(tk.Toplevel):
    ...
    def build_ui(self):
        ...
        self.add_form_row(form_frame, 1, "Quality Tool No:", "", "tool_no")
        ...
        self.fields["tool_no"].trace_add("write", self.auto_fill)
    ...
    def auto_fill(self, *args):
        """Auto-fills the INVENTORY_NUMBER field if tool_no matches the database."""
        current_tool_no = self.fields["tool_no"].get().strip()
        
        if current_tool_no in self.parent.fixture_jig_db:
            data = self.parent.fixture_jig_db[current_tool_no]
            self.fields["inv_no"].set(data["INVENTORY_NUMBER"])
        else:
            self.fields["inv_no"].set("")

    def execute_manual_print(self):
        tool_no = self.fields["tool_no"].get().strip()
        ...
        if not tool_no:
            messagebox.showwarning(
                "Missing Data",
                "Please enter a valid Quality Tool Number."
            )
            return

        if tool_no not in self.parent.fixture_jig_db:
            messagebox.showwarning(
                "Not Found",
                "The Quality Tool Number was not found in Fixture-Jig_db.csv."
            )
            return
        ...
        if success:
            ...
            self.fields["tool_no"].set("") # clear for next scan/entry
        else:
