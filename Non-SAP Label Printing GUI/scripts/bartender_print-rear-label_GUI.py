...
LABELFILE   = "Rear label RRC2020-DR.BTW"
LOG_FILE = r"C:\bt_watchedfolder_qr_print\qr_print_log.csv"

HEADER = ["PRINTERNAME", "LABELFILE", "CUSTOMER_REV"]

# =========================
# PRINT LOGIC
# =========================
def generate_print_file(customer_rev, copies):
    ...
    mundat_str = ""
    expdat_str = ""
    row = [PRINTERNAME, LABELFILE, customer_rev]
...
# GUI APP
# =========================
class RearLabelForm(tk.Tk):
    def __init__(self):
        super().__init__()
        self.title("Print Form - Rear Label (RRC2020-DR)")
    ...
    def build_ui(self):
        ...
        tk.Label(
            header,
            text="Print - Rear Label (RRC2020-DR)",
        ...
        # Customer Revision Input Section
        rev_frame = tk.Frame(card, bg=self.colors["card"])
        rev_frame.pack(pady=(5, 5))
    ...
    def execute_print(self):
        customer_rev = self.rev_var.get().strip()
        if not customer_rev:
            messagebox.showwarning("Missing Data", "Please enter a Customer Revision.")
            return
        ...
        success, msg = generate_print_file(customer_rev, copies)

        if success:
            messagebox.showinfo("Success", f"Sent {copies} copy/copies of Rear Label (RRC2020-DR) to {PRINTERNAME}.")
            self.destroy()
        else:
